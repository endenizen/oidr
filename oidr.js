...
function calculate() {
  rows = $('.history table tr');

  rows.each(function() {
    var winner = $(this).find('td:eq(0)').text();
    var loser = $(this).find('td:eq(2)').text();

    if(!getPerson(winner)) {
      people.push(new Person(winner));
    }
    if(!getPerson(loser)) {
      people.push(new Person(loser));
    }
  });
...
function lookAtAllGames() {
  while(curGame < games.length) {
    lookAtNextGame();
    curGame++;
  }
  makeGrid();
...
function lookAtNextGame() {
  ...
  var dt = parseDate(dateCell.text());
